...
public class FirstBossController : MonoBehaviour {
	...
	void Update () {
        ...
        if (LeftFlag && this.transform.position.x > -5)
        {
            time += Time.deltaTime;
        ...
        if (RightFlag && this.transform.position.x < 5)
        {
            time += Time.deltaTime;
            this.transform.Translate(0.5f, 0, 0);
            //時間が経てば元に戻る
        ...
        if (isDamaged)
        {
            ...
            // 点滅の処理をさせる。
            float level = Mathf.Abs(Mathf.Sin(Time.time * 10));
            this.GetComponent<SpriteRenderer>().color = new Color(0f, 0f, 0f, level);
