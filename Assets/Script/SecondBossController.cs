...
public class SecondBossController : MonoBehaviour {
    ...
    public AudioClip HPZero; //HPが0になった時のSE
    ...
    IEnumerator StartFirst()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            Mizutama.transform.position = new Vector2(this.transform.position.x -3.0f , this.transform.position.y -1.5f );
    ...
    {
        //音を鳴らす
        audiosource.PlayOneShot(HPZero, 0.8f);
        isEnd = true;//死亡フラグON
